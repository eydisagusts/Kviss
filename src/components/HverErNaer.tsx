...
const HverErNaer = ({ team, teamName, onSubmit }: HverErNaerProps) => {
  const [answers, setAnswers] = useState(Array(1).fill(""));
  const [points, setPoints] = useState(Array(1).fill(0));
    ...
    <div className="flex flex-col items-center mb-8 border-2 rounded-lg border-white p-4">
      ...
      <div className="grid grid-cols-1 gap-8">
        {answers.map((answer, index) => (
          <div key={`HverErNaer-${team}-${index}-${answer}`} className="flex items-center justify-between">
